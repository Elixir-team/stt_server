#!/usr/bin/env python3
import time
from fastapi import FastAPI, WebSocket, UploadFile, File, UploadFile, Form
import uvicorn
import whisper
import re
...
def format_bytes_to_np_array(data: bytes):
    if model.device.type == "cpu":
        ...
        return np.frombuffer(data, dtype=np.int16).flatten().astype(np.float16) / 32768.0


def get_audio_as_numpy(file: UploadFile):
...
async def rest_endpoint(language: str = Form(...), file: UploadFile = File(...)):
    audio_array = get_audio_as_numpy(file)
